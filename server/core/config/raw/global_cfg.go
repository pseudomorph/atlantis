...
// reuses the server's redis flags; only the key prefix and object TTL differ.
...
func (p PlanStoreConfig) Validate() error {
	...
	case "redis":
		if p.Redis.TTL != "" {
			if _, err := time.ParseDuration(p.Redis.TTL); err != nil {
	...
	return valid.ExternalStores{
		PlanStore: valid.PlanStoreConfig{
			...
			Redis: valid.RedisStoreConfig{
				Prefix: e.PlanStore.Redis.Prefix,
				TTL:    redisTTL,
			},
		},
	}
